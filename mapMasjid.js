 ...
 * Fitur: Mihrab 4m, 2 Saf Karpet Lesehan Depan, Void Lantai 2, & Kubah Kopong Biru Langit
 */
...
export function loadMapMasjid() {
  ...
  // Material Kubah Kopong Biru Langit (DoubleSide agar rongga dalam terlihat jelas)
  const matSkyBlueDome = new THREE.MeshStandardMaterial({ 
    color: 0x38bdf8, 
    roughness: 0.35, 
  ...
  // =========================================================================
  // 2. DIMENSI UTAMA RUANG MASJID
  // =========================================================================
  const roomW = 20.00;
  const roomD = 16.00;
  const floor1H = 3.80;
  const floor2H = 3.40;
  const totalWallH = floor1H + floor2H; // 7.20 Meter

  // A. Lantai Keramik Putih Mengkilap Seluas Ruangan
  ...
  // B. 2 Baris Saf Karpet Hijau Lesehan (Mulai dari Dinding Kiblat)
  const carpetW = 16.00;
  const safDepth = 1.25;
  ...
  const floorNoticeLabel = createTextLabel('SAF 1 & 2 (LESEHAN KARPET HIJAU)', 'AREA BELAKANG: LANTAI PUTIH BERSIH', '#facc15');
  floorNoticeLabel.rotation.x = -Math.PI / 2;
  floorNoticeLabel.position.set(0, 0.02, -2.50);
  floorNoticeLabel.scale.set(0.9, 0.9, 0.9);
  mapGroup.add(floorNoticeLabel);

  // =========================================================================
  // 3. CERUK MIHRAB TEMPAT IMAM (PANJANG 4.00 METER)
  // =========================================================================
  const mihrabW = 4.00;
  const mihrabH = 3.40;
  const mihrabDepth = 4.00; // Panjang 4 meter
  const mihrabZ = -roomD / 2; // -8.00 m
  ...
  // =========================================================================
  // 4. DINDING UTAMA KELILING (TINGGI 7.20 M)
  // =========================================================================
  ...
  // =========================================================================
  // 5. 4 PILAR UTAMA & VOID LANTAI 2
  // =========================================================================
  const pillarX = 5.20;
  const pillarZ = 4.00;
  const voidW = pillarX * 2; // 10.40 Meter
  const voidD = pillarZ * 2; // 8.00 Meter
  ...
  [
    ...
    const pMid = new THREE.Mesh(new THREE.BoxGeometry(0.80, 0.25, 0.80), matGoldTrim);
    pMid.position.set(px, floor1H, pz);
    mapGroup.add(pMid);
  });

  // Lantai 2 Mezzanine
  const slabThick = 0.25;
  const slabY = floor1H - (slabThick / 2);
  ...
  // =========================================================================
  // 6. PAGAR PEMBATAS (RAILING) KELILING VOID LANTAI 2
  // =========================================================================
  const railingH = 0.90;
  const railingY = floor1H + (railingH / 2);
  ...
  // =========================================================================
  // 7. KUBAH ATAP KOPONG BIRU LANGIT (HOLLOW SKY BLUE DOME)
  // =========================================================================
  const roofCeilingY = totalWallH; // Y = 7.20 Meter
  const domeRadius = 4.80;

  // A. Plafon Atas Berlubang (Frame Void Plafon Atap)
  ...
  // B. Ring Cincin Dasar Kubah (Open-Ended, Tanpa Tutup Padat)
  const ringGeo = new THREE.CylinderGeometry(domeRadius, domeRadius, 0.35, 32, 1, true); // true = Tanpa tutup disk
  ...
  // C. Setengah Bola Kubah Kopong (Membubung ke Atas / +Y, Kopong di Sisi Bawah)
  ...
  // Garis-garis Rangka Kubah Putih Bersih
  const domeEdges = new THREE.LineSegments(new THREE.EdgesGeometry(domeGeo), lineDomeRibs);
  domeMesh.add(domeEdges);

  // D. Lampu Gantung (Chandelier) Menjuntai dari Puncak Kubah
  const wireChain = new THREE.Mesh(
    new THREE.CylinderGeometry(0.02, 0.02, 4.8, 8),
  ...
  // =========================================================================
  // 8. LAYAR BACKDROP & MEJA LESEHAN SEMINAR (TANPA PANGGUNG)
  // =========================================================================
  ...
  const screenMesh = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ map: createSeminarScreenTexture(), roughness: 0.3 })
  );
  screenMesh.position.set(0, 2.2, -roomD / 2 + 0.2);
  mapGroup.add(screenMesh);

  // Meja Lesehan Rendah Pemateri & Moderator (Tinggi 0.35m)
  ...
  // Meja Lesehan Sosialisasi Kampus Sayap Kanan (Unisla, Umla, Polteksi)
  ...
  // Kotak Pohon Wishlist Siswa di Sayap Kiri
  ...
  // Titik Spawn Siswa: Di atas Lantai Putih Belakang menghadap ke Mihrab/Kiblat (-Z)
  const spawnPoint = {
    x: 0,
    y: 1.65,
    z: roomD / 2 - 2.5,
